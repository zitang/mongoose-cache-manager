...
module.exports = function mongooseCache(mongoose, options, cb) {
    ...
    var proto = mongoose.Query.prototype,
        ...
        emptyObj = {},
        Q = defaultOptions.Q,
        modelProto = mongoose.Model.prototype;
    ...
    proto.exec = function (operation, cb) {
        ...
        cache.get(collectionKey, queryKey, function (err, collectionData, queryData) {
            ...
            if (err) {
                // ignore it, treawt it as a cache missing
                //if (callback) {
                //    return callback(err);
                //} else {
                //    deferred.reject(err);
            ...
            if (queryData) {
                if (callback) {
                    ...
                    deferred.resolve(queryData.docs);
                }
            }
    ...
    ['save'].forEach(function (op) {
        modelProto['_' + op] = modelProto[op];
        modelProto[op] = function () {
            this._touchCollectionCheck.apply(this, arguments);

            return modelProto['_' + op].apply(this, arguments);
        };
    });
    ...
    modelProto._touchCollection = proto._touchCollection = function (name) {
    ...
    modelProto._touchCollectionCheck = function () {
        var callback = arguments.length ? arguments[arguments.length - 1] : false;

        // mquery doesn't run the write unless there is a callback so unless
        // there is one here, we do not want to touch the collection data.
        if (callback) {
            this._touchCollection(this.collection.name);
        }
    };
